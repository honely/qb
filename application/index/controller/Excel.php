...
//
//        //我的数据信息
//        $contentRows = Db::table('qbl_customer')->select();
//        if($contentRows){
...
//    }
    ...
    public function outExcelRecharge() {
        $data=Db::table('qbl_customer')->field('cus_id,cus_name,cus_phone')->select();
        $field = array(
            'A' => array('cus_id', 'ID'),
    ...
    public function emportBatch () {
        ...
        //数据字段
        $db = Db::getConnection();
        $fields = $db->getFields("qbl_customer");
        $xlsCell = [];
        $i = 0;
        ...
        if (empty($excid)) {
            $xlsData =Db::table('qbl_customer')->select(); //全部导出
        } else {
            $xlsData =Db::table('qbl_customer')->where('cus_id','IN',$excid)->select(); //选中导出
        }
